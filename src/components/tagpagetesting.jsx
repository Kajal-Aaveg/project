...
const Tag = ({ tag, onClick }) => (
  <div
    className="text-center p-1 bg-white hover:bg-indigo-950 hover:text-white text-black border rounded mt-[10px] max-sm:w-full max-sm:mb-2 cursor-pointer"
    onClick={() => onClick(tag)}
  >
    {tag}
  </div>
...
function BlogTest() {
 ...
 const Navigate=useNavigate();
  ...
  const handleClickk = (tag) => {
    setSelectedTag(tag);
  };

  // Filter data based on the selected tag
  const filteredData = selectedTag
    ? favData.filter((data) => data.tags.split(',')[0] === selectedTag)
    : favData;
  ...
  return (
    <>
      ...
      <div className="mx-10 mt-20 max-sm:mt-10">
        ...
        {/* Display the grid of tags */}
        <div className="grid grid-cols-3 gap-1">
          {favData.map((itemTags, index) => (
            <Tag key={index} tag={itemTags.tags.split(',')[0]} onClick={handleClickk} />
          ))}
        </div>
      </div>

      {/* Display the filtered blogs based on the selected tag */}
      <div className="w-[60%] px-10 max-sm:w-full">
        <h1 className="font-bold pt-6">
          {selectedTag ? `Blogs related to '${selectedTag}'` : 'All Blogs'}
        </h1>
        <div className="flex pb-3">
          {/* ... Your existing code for the line */}
        </div>
        <div className="w-full mt-2">
          <div className="text-[11pt] font-['Average_Sans'] ">
            {filteredData.map((data, index) => (
              <div key={index}>
                {/* Your existing code for displaying blogs */}
              </div>
            ))}
          </div>
        </div>
